...
export default function OurApproach({ onScrollToSection }: OurApproachProps) {
  const steps = [
    ...
    {
      label: "Support",
      desc: "We stay by your side to make sure everything runs smoothly.",
      ...
      <div className="space-y-12 max-w-5xl mx-auto">
        ...
        <div className="space-y-4 text-center max-w-3xl mx-auto">
          ...
          <p className="text-slate-500 text-xs sm:text-sm max-w-xl mx-auto leading-relaxed font-semibold">
            Instead of offering general packages, we follow a simple, clear, and quiet path to design what fits you.
          </p>
        </div>
        ...
        <div className="relative pt-6">
          ...
          {/* Background horizontal connector line on desktop */}
          <div className="hidden lg:block absolute top-[43px] left-[8%] right-[8%] h-0.5 bg-slate-100 z-0"></div>

          <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-8 lg:gap-4 relative z-10 text-center">
            {steps.map((step, idx) => (
              <div 
                key={idx} 
              ...
              >
                ...
                <div className="relative">
                  <div className="h-14 w-14 rounded-2xl bg-white border border-slate-200/80 shadow-sm flex items-center justify-center transition-all duration-300 group-hover:scale-105 group-hover:border-[#2563EB]/40 group-hover:shadow-md z-10 relative">
                    {step.icon}
                  </div>
                  {/* Step counter badge */}
                  <span className="absolute -top-1.5 -right-1.5 bg-[#0B1F3A] text-white text-[8px] font-bold h-4 w-4 rounded-full flex items-center justify-center font-mono">
                    {idx + 1}
                  </span>
                </div>
                ...
                {idx < steps.length - 1 && (
                  <div className="lg:hidden text-slate-300 pt-2">
