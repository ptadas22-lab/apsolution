      ...
      <div className="max-w-5xl mx-auto px-4 space-y-16">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {recommendations.map((rec, idx) => (
            <div key={idx} className="bg-slate-50 rounded-3xl p-8 border-2 border-slate-100 hover:border-blue-200 transition-colors shadow-sm flex flex-col items-center text-center gap-4 animate-in slide-in-from-bottom-8 fade-in duration-700">
              
              <div className="space-y-1">
                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest font-mono">
                  Goal
                </span>
                <h3 className="text-sm font-black text-[#0B1F3A] leading-tight">
                  {rec.possibleImprovement}
                </h3>
              </div>
              
              <div className="bg-white border-2 border-[#2563EB]/20 rounded-full p-1.5 text-[#2563EB] shadow-sm my-1">
